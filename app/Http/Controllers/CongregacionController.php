...
class CongregacionController extends Controller {
    ...
    function informeMensual($idCarpeta) {
        ...
        if (isset($infoCarpeta->id)) {
            ...
            $inicioMes = $this->getInicioMes($infoCarpeta->fechaCompuesta);
            ...
            $pdf = PDF::loadView('pdf.informeMensual', compact("infoCarpeta", "infoCongregacion", "totalEntradaCongregacion", "lstGastos", "celdasVaciasGastos", "totalGastos", "sobrante", "fondosFinMes", "totalObraMundial", "totalRecibido", "totalDesembolsos", "fondosFinMes2", "mes", "totalResolucionSalones", "inicioMes", "mesAnnio"))->setPaper('a4', 'portrait');
            return $pdf->stream();
        }
    }
    ...
    function hojadeCuentas($idCarpeta) {
        ...
        if (isset($infoCarpeta->id)) {
            ...
            $pdf = PDF::loadView('pdf.hojaCuentas', compact("celdasVaciasGastos", "lstGastos", "totalEntradaCongregacion", "finalizacionMes", "mes", "infoCongregacion", "cajaFinal1", "cajaFinal2", "totalCaja", "totalSalida", "totalEntrada", "sumResolucionObra", "infoCarpeta", "totalResolucionSalones", "celdasVacias", "recibo", "totalObraMundial", "totalGastos", "inicioMes", "totalConsignaciones"))->setPaper('a4', 'landscape');
            return $pdf->stream();
        }
    }
    ...
    function remesa($idCarpeta) {
        ...
        if (isset($infoCarpeta->id)) {
            ...
            $pdf = PDF::loadView('pdf.remesa', compact("infoCarpeta", "infoCongregacion", "mesAnnio", "totalResolucionSalones", "total", "numeroHojasConsignacion", "totalObraMundial"))->setPaper('A4', 'landscape');
            return $pdf->stream();
        }
    }
    ...
    function guardarCarpeta(Request $request) {
        ...
        $fechaCompuesta = strtotime("$request->mes/01/$request->annio");
        $carpeta = Carpeta::where("fechaCompuesta", $fechaCompuesta)->get();
        if (count($carpeta) > 0) {
            ...
            $carpeta->fechaCompuesta = $fechaCompuesta;
...
//    var_dump($inicioMes);die;
    ...
    function migrarFechaCompuesta(){
        $listCarpetas = Carpeta::all();
        foreach ($listCarpetas as $carpeta){
            $fechaCompuesta = strtotime("$carpeta->mes/01/$carpeta->annio");   
            $infoCarpeta = Carpeta::find($carpeta->id);
            $infoCarpeta->fechaCompuesta = $fechaCompuesta;
            $infoCarpeta->save();
        }
    }
